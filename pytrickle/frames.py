...
class VideoFrame(InputFrame):
    ...
    def to_ml_format(self) -> torch.Tensor:
        """Convert to ML model format: [B,C,H,W] RGB [0,1].
        
        Standard format for PyTorch models, including Stable Diffusion.
        Converts from the internal [B,H,W,C] format to [B,C,H,W].
        
        Returns:
            Tensor in [B,C,H,W] format with RGB channels, normalized [0,1]
        """
        tensor = self.tensor
        ...
        if tensor.dim() == 3:  # [H,W,C]
            ...
            raise ValueError(f"Expected 3D or 4D tensor, got {tensor.dim()}D with shape {tensor.shape}")
        
        # Convert from [B,H,W,C] to [B,C,H,W] if needed
        if tensor.shape[-1] in [1, 3, 4]:  # Channels last format
            tensor = tensor.permute(0, 3, 1, 2)  # [B,H,W,C] -> [B,C,H,W]
        
        return tensor
    
    def to_sd_format(self) -> torch.Tensor:
        """Convert to Stable Diffusion format: [B,C,H,W] RGB [-1,1].
        
        Stable Diffusion models typically expect input in [-1,1] range.
        
        Returns:
            Tensor in [B,C,H,W] format with RGB channels, normalized to [-1,1]
        """
        tensor = self.to_ml_format()
        ...
        if tensor.dtype in [torch.float32, torch.float64]:
            ...
            tensor = (tensor.float() / 255.0) * 2.0 - 1.0
        
        return tensor
    
    def to_opencv_format(self) -> np.ndarray:
        """Convert to OpenCV format: [H,W,C] BGR uint8.
        
        OpenCV uses BGR channel order and uint8 values [0,255].
        
        Returns:
            NumPy array in [H,W,C] BGR format, uint8 [0,255]
        """
        tensor = self.tensor
        ...
        if tensor.dim() == 4:
            ...
            raise ValueError(f"Expected 3D tensor, got {tensor.dim()}D with shape {tensor.shape}")
        
        # Convert to numpy
        if tensor.dtype in [torch.float32, torch.float64]:
            # Convert [0,1] to [0,255] uint8
            tensor_np = (tensor * 255).clamp(0, 255).to(torch.uint8).cpu().numpy()
        else:
            tensor_np = tensor.clamp(0, 255).to(torch.uint8).cpu().numpy()
        
        # Convert RGB to BGR for OpenCV
        if tensor_np.shape[-1] == 3:  # RGB channels
            tensor_np = tensor_np[..., [2, 1, 0]]  # RGB -> BGR
        
        return tensor_np
    
    def to_comfy_format(self) -> torch.Tensor:
        """Convert to ComfyUI format: [B,H,W,C] RGB [0,1].
        
        ComfyUI typically expects tensors in [B,H,W,C] format.
        This is the same as our internal format.
        
        Returns:
            Tensor in [B,H,W,C] format with RGB channels, normalized [0,1]
        """
        tensor = self.tensor
    ...
    @classmethod
    def from_opencv(cls, bgr_array: np.ndarray, timestamp: int = 0, time_base: Fraction = None) -> 'VideoFrame':
        """Create VideoFrame from OpenCV BGR array.
        
        Args:
            bgr_array: NumPy array in [H,W,C] BGR format
            timestamp: Frame timestamp
            time_base: Time base for timestamp
            
        Returns:
            VideoFrame with RGB tensor in [B,H,W,C] format
        """
        if time_base is None:
            time_base = Fraction(1, 1)
        
        # Convert BGR to RGB
        if bgr_array.shape[-1] == 3:
            rgb_array = bgr_array[..., [2, 1, 0]]  # BGR -> RGB
        else:
            rgb_array = bgr_array
        
        # Convert to tensor and normalize
        if rgb_array.dtype == np.uint8:
            tensor = torch.from_numpy(rgb_array.astype(np.float32)) / 255.0
        else:
            tensor = torch.from_numpy(rgb_array.astype(np.float32))
            if tensor.max() > 1.0:  # Assume [0,255] range
                tensor = tensor / 255.0
    ...
    @classmethod 
    def from_ml_tensor(cls, ml_tensor: torch.Tensor, timestamp: int = 0, time_base: Fraction = None) -> 'VideoFrame':
        """Create VideoFrame from ML model tensor format.
        
        Args:
            ml_tensor: Tensor in [B,C,H,W] or [C,H,W] format
            timestamp: Frame timestamp  
            time_base: Time base for timestamp
            
        Returns:
            VideoFrame with tensor in [B,H,W,C] format
        """
        if time_base is None:
            time_base = Fraction(1, 1)
        ...
        if tensor.shape[1] in [1, 3, 4]:  # Channels first format
            tensor = tensor.permute(0, 2, 3, 1)  # [B,C,H,W] -> [B,H,W,C]
        
        return cls(tensor, timestamp, time_base)
    
    @classmethod
    def from_comfy_tensor(cls, comfy_tensor: torch.Tensor, timestamp: int = 0, time_base: Fraction = None) -> 'VideoFrame':
        """Create VideoFrame from ComfyUI tensor format.
        
        Args:
            comfy_tensor: Tensor in ComfyUI format (typically [B,H,W,C])
            timestamp: Frame timestamp
            time_base: Time base for timestamp
        ...
        # ComfyUI format is the same as our internal format
        return cls(comfy_tensor, timestamp, time_base)
